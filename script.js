...
function selectedVolume(){return Number(document.getElementById("soundVolume").value)/25}
...
function chime(){stopSound();ctx=ctx||new(window.AudioContext||window.webkitAudioContext)();gain=ctx.createGain();gain.gain.value=1;gain.connect(ctx.destination);const play=()=>{const o=ctx.createOscillator(),g=ctx.createGain();o.type="sine";o.frequency.value=[523,659,784,880][Math.floor(Math.random()*4)];g.gain.setValueAtTime(0,ctx.currentTime);g.gain.linearRampToValueAtTime(selectedVolume()*.3,ctx.currentTime+.03);g.gain.exponentialRampToValueAtTime(.0001,ctx.currentTime+2.6);o.connect(g).connect(gain);o.start();o.stop(ctx.currentTime+2.7)};play();timer=setInterval(play,2200)}
soundToggle.addEventListener("click",()=>{const open=panel.classList.toggle("open");soundToggle.setAttribute("aria-pressed",open);soundToggle.innerHTML=open?"<span>◉</span> Choose ambience":"<span>◌</span> Ambient sound";if(!open)stopSound()});document.querySelectorAll("[data-sound]").forEach(b=>b.addEventListener("click",()=>{document.querySelectorAll("[data-sound]").forEach(x=>x.classList.remove("active"));b.classList.add("active");b.dataset.sound==="wood"?chime():noise(b.dataset.sound)}));document.getElementById("soundVolume").addEventListener("input",()=>{if(gain&&ctx&&source)gain.gain.linearRampToValueAtTime(selectedVolume(),ctx.currentTime+.15)});
